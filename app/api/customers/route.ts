...
export async function GET(request: NextRequest) {
  try {
    ...
    if (roleId === 'primary_executive') {
      query = query.eq('primary_executive_id', authResult.userId);
      ...
      query = query.eq('primary_executive_id', currentUser.manager_id).eq('secondary_executive_id', authResult.userId);
    }
    ...
    if (customerIds.length > 0) {
      const { data: invoices } = await supabaseAdmin
        .from('invoices').select('customer_id, balance_due')
        .in('customer_id', customerIds).neq('status', 'Cancelled');
      if (invoices) {
        for (const inv of invoices) {
          balanceMap[inv.customer_id] = (balanceMap[inv.customer_id] || 0) + (inv.balance_due || 0);
        }
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.email) {
      const { data: dup } = await supabaseAdmin.from('customers').select('id').eq('email', body.email).maybeSingle();
      if (dup) return NextResponse.json({ error: 'Customer with this email already exists' }, { status: 400 });
    }

    let primaryExecutiveId: string | null = null;
    let secondaryExecutiveId: string | null = null;

    if (roleId === 'primary_executive') {
      primaryExecutiveId = authResult.userId;
      if (body.secondary_executive_id) {
        const { data: sub } = await supabaseAdmin.from('users').select('manager_id, role').eq('id', body.secondary_executive_id).single();
        ...
        secondaryExecutiveId = body.secondary_executive_id;
      }
    } else if (roleId === 'secondary_executive') {
      if (!currentUser.manager_id) return NextResponse.json({ error: 'Secondary executive is not assigned to a primary executive' }, { status: 400 });
      primaryExecutiveId = currentUser.manager_id;
      secondaryExecutiveId = authResult.userId;
    } else if (roleId === 'admin') {
      if (body.primary_executive_id) {
        const { data: pe } = await supabaseAdmin.from('users').select('role').eq('id', body.primary_executive_id).single();
        ...
        primaryExecutiveId = body.primary_executive_id;
      }
      if (body.secondary_executive_id) {
        const { data: se } = await supabaseAdmin.from('users').select('role, manager_id').eq('id', body.secondary_executive_id).single();
        ...
        if (primaryExecutiveId && se.manager_id !== primaryExecutiveId)
    ...
    const { data, error } = await supabaseAdmin.from('customers').insert({
      ...
      primary_executive_id: primaryExecutiveId, secondary_executive_id: secondaryExecutiveId,
