    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">Track SE daily sales and locations</p>
        </div>
        <Button onClick={loadSales} variant="outline" size="sm" disabled={loading}>
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-2xl font-bold">{salesWithLocation.length}</div>
                <div className="text-xs text-gray-500">With GPS</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {salesWithLocation.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-gray-500">
                ...
                <p className="text-sm">Sales logged without GPS won&apos;t appear on the map</p>
              </div>
            ) : (
          ...
          <CardContent className="pt-4">
            {loading ? (
              ...
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="text-lg font-medium">No field sales found</p>
                      ...
                      <TableRow key={sale._id}>
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            <span className="font-medium text-sm">{sale.se_name || '—'}</span>
                          </div>
                        </TableCell>
                        <TableCell className="text-sm">{sale.product_name || '—'}</TableCell>
                        <TableCell className="hidden sm:table-cell text-sm">{sale.distributor_name || '—'}</TableCell>
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <MapPin className="h-5 w-5 text-green-600" />SE Location While Logging Sale
            </DialogTitle>
            <DialogDescription>
              {selectedSale?.se_name} — {selectedSale?.product_name} — {selectedSale ? formatIST(selectedSale.created_at) : ''} IST
            </DialogDescription>
          </DialogHeader>
          {selectedSale?.location_lat && selectedSale?.location_lng && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><strong>SE:</strong> {selectedSale.se_name}</div>
                <div><strong>Distributor:</strong> {selectedSale.distributor_name}</div>
                <div><strong>Product:</strong> {selectedSale.product_name}</div>
                <div><strong>Qty:</strong> {selectedSale.quantity_sold} {selectedSale.unit}</div>
                <div><strong>Amount:</strong> ₹{selectedSale.sale_amount.toLocaleString()}</div>
                {selectedSale.retailer_name && <div><strong>Retailer:</strong> {selectedSale.retailer_name}</div>}
              </div>
              <Suspense fallback={<div className="h-[400px] bg-gray-100 rounded-lg flex items-center justify-center text-sm text-gray-500">Loading map...</div>}>
                <SalesMap
                  markers={[{
                    ...
                    popup: `${selectedSale.se_name}<br/>₹${selectedSale.sale_amount.toLocaleString()}<br/>${formatIST(selectedSale.created_at)} IST`,
                    color: 'green',
                  }]}
                  zoom={14}
                />
